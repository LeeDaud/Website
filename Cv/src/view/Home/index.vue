...
const downloadResume = () => {
  window.open(`${baseURL}/cv/resume/download`, '_blank', 'noopener')
}
...
onMounted(async () => {
  initTheme()
  try {
    const [infoRes, expRes, skillRes, resumeStatusRes] = await Promise.all([
      getPersonalInfo(),
      getExperiences(),
      getSkills(),
      getResumeStatus()
    ...
    resumeAvailable.value = Boolean(resumeStatusRes.data.data?.available)
  } catch (e) {
              ...
              <div class="tl-info">
                <h3>
                  <a
                    ...
                    :href="toExternalUrl(item.projectLink)"
                    target="_blank"
                    rel="noopener"
