...
const homeUrl = computed(() => {
  const fromProfile = normalizeUrl(blogStore.personalInfo?.website)
  if (fromProfile) return fromProfile
  return import.meta.env.DEV ? 'http://localhost:5175' : '/'
})
...
const navItems = computed(() => [
  ...
  { label: '项目', icon: 'icon-lianjie', to: '/projects' },
  ...
  <header class="site-header" :class="{ scrolled, dark: isDark }">
    <div class="header-inner">
      <div class="header-left">
        <router-link to="/" class="site-title">LeeDaud's Blog</router-link>
